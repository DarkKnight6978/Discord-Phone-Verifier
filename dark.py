...
class Logger:
    def __init__(self, level: LogLevel = LogLevel.DEBUG):
        self.level = level
        self.prefix = "\033[96m[\033[38;5;220mDarkOP\033[96m] "
        self.WHITE = "\u001b[37m"
...
async def check_country_selected(tab, expected_country):
    js = """
        (() => {
            const selected = document.querySelectorAll('input[class*="searchInput__3f413"]');
            return selected ? Array.from(selected).map(input => input.value.trim().toLowerCase()) : [];
        })()
    """
    selected_values = await tab.evaluate(js)
    return expected_country.lower() in selected_values
...
def send_notification(title, message):
    ...
    try:
        notification = Notify()
        notification.application_name = "Dark Verifier"
...
async def login_discord_with_token(token, password, mail):
    ...
    if not await find_and_click_verify_button(tab):
        ...
        return False
    try:
        pg.press('tab', presses=6, interval=1)
        await asyncio.sleep(1)
        pg.write(COUNTRY.capitalize(), interval=0.05)
        await asyncio.sleep(1)
        pg.press('enter')
        if await check_country_selected(tab, COUNTRY):
            log.info(f"✅ Selected country: {COUNTRY}")
        else:
            log.error(f"❌ Country selection mismatch: expected {COUNTRY}, trying again")
            pg.press('tab', presses=3, interval=0.5)
            await asyncio.sleep(1)
            pg.write(COUNTRY.lower(), interval=0.05)
            await asyncio.sleep(1)
            pg.press('enter')
            if await check_country_selected(tab, COUNTRY):
                log.info(f"✅ Selected country: {COUNTRY}")
            else:
                log.error(f"❌ Country selection mismatch: expected {COUNTRY}, returning")
                return

    except Exception as e:
        log.error(f"❌ Country selection error: {e}")
        return
    ...
    send_notification("Dark OP", "⚠️ Please solve the CAPTCHA!")
